...
static NSString *SG_FOCUS_ITEM_ASS_KEY = @"cn.nfsysu";  //扩展uiview 增加点击属性
//参数key是个指针，由此意味着ObjectTagKey设置和获取方法必须传入相同指针，否则在其他地方调用get时生成新的key字符串作为参数，会不起作用
...
- (void)setupViews
{
    ...
    CGSize size = CGSizeMake(100, 24);
    //这里控制pageControl 的位置   //self.bounds.size.width *.5 - size.width *.5,
    ...
    [self addSubview:_pageControl]; //显示当前页
    ...
    for (int i = 0; i < imageItems.count; i++) {
        ...
        //添加标题栏，这里是背景栏
        UILabel * lbltitle = [[UILabel alloc] initWithFrame:CGRectMake(i * _scrollView.frame.size.width+16, _scrollView.frame.size.height-28.0, _scrollView.frame.size.width-16, 28.0)];
        ...
        UIView *titleBgView=[[UIView alloc] initWithFrame:CGRectMake(i * _scrollView.frame.size.width, _scrollView.frame.size.height-28.0, _scrollView.frame.size.width, 28.0)];
        [titleBgView setBackgroundColor:[UIColor colorWithWhite:1.0 alpha:.3]];
        
        //[_scrollView setBackgroundColor:[UIColor grayColor]];
        [_scrollView addSubview:imageView];
        [_scrollView addSubview:lbltitle];
        [_scrollView.superview addSubview:titleBgView];
    }
...
-(void)clickPageImage:(UIButton *)sender{
    [self.delegate foucusImageFrame:self didSelectItem:nil];
}
